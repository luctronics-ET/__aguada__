...
async function startServer() {
  try {
    ...
    try {
      ...
      logger.warn(
        "[Serial Bridge] Erro ao conectar na porta serial (porta pode não existir):",
        err.message
      );
      logger.warn(
        "[Serial Bridge] Sistema funcionará sem gateway serial por enquanto"
      );
    }
    ...
    server.listen(PORT, () => {
      ...
      logger.info(
        `📡 Serial Bridge: ${serialBridge.portPath} @ ${serialBridge.baudRate} baud`
      );
    });
  } catch (error) {
